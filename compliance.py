...
async def log_event(session_id: str, event_type: str, content: Any) -> None:
    await execute(
        "INSERT INTO compliance_log(session_id, event_type, content, timestamp) VALUES (?, ?, ?, ?)",
...
async def get_session_log(session_id: str) -> list[dict[str, Any]]:
    return await fetch_all(
        "SELECT session_id, event_type, content, timestamp FROM compliance_log WHERE session_id = ? ORDER BY id ASC",
        (session_id,),
    )
